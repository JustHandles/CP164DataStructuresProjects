...
def clean_list(values):
    ...
    for x in range(len(values)):
        if values[x] not in bad_list:
            bad_list.append(values[x])
    values = bad_list
    print(values)
    return None
...
def list_subtract(minuend, subtrahend):
    ...
    while n < len(minuend):
        subcheck = 0
        while subcheck < len(subtrahend):
            if subtrahend[subcheck] == minuend[n]:
                minuend.pop(n)
                n -= 1
                break
            subcheck += 1
...
def matrixes_add(a, b):
    ...
    for x in range(len(a)):
        c1 = []
        for l in range(len(a[x])):
            c_num = int(a[x][l] + b[x][l])
            c1.append(c_num)
        c.append(c1)
    return c
